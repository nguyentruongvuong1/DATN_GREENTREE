...
export default function Cart() {
  ...
  const Tongtien = useCallback(() =>
    cart.reduce(
      (tong, pr) => tong + pr.so_luong * (pr.price_sale === 0 ? pr.price : pr.price_sale),
      0
    ), [cart]);
  ...
  const applyVoucher = async () => {
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/checkvoucher`,
        { code: VoucherCode }
      );
      ...
      <div className={styles.container}>
            ...
            <tbody>
              {cart.map((pr) => (
                <tr key={pr.id}>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (value > pr.inventory_quantity) {
                          ...
                          message.warning(
                            `Số lượng tối đa: ${pr.inventory_quantity}`
                          );
                        }
                  ...
                  <td>{(pr.so_luong * pr.price_sale === 0 ? pr.price : pr.price_sale ).toLocaleString("vi")} VNĐ</td>
                  <td>
                    <button
        ...
        <div className={styles.container_tongdonhang}>
          ...
          <div className={styles.voucher}>
            ...
            <p className={styles.voucher_note}>
              * Mỗi đơn hàng chỉ sử dụng được 1 voucher
            </p>
            <input
              type="text"
          ...
          <p>
            <strong>Thành tiền:</strong> {Number(Tongtien()).toLocaleString("vi")} VNĐ
          </p>
          <p>
